...
=for html <!-- program last modified Time-stamp: "2012-11-09 15:06:33 MST"
  Regenerate this HTML file with:
   perldoc -ohtml -dcrontab2english.html  crontab2english
 -->
 <p align=center><i>[ <a href="crontab2english">Download crontab2html
 here</a> -- do "Save As..." ]</i></p>
...
running somewhere.  We would hope that these would all basically
...
As of time of writing (2012-11-09), the only substantial bug that has
been reported since 2001-07-29 is that in the weekday field,
I<abbrv-abbrv> things such as "tue-sat", aren't accepted.  I'm now looking
into this.  I'm notably considering that I don't want to accept it
unless most cron daemons accept it too-- especially considering the corner
cases like "sat-tue" or "mon-fri/2".

Until then, using numerics gets the job done: 2-5 (which means Tuesday
to Saturday).

I<(Pseudobugs/Harrumphs:)>

The expression of C<i-j/y> things can still kind of rub me the wrong
way, I<no matter> what way I phrase it.  So I'm not sure whether
"every 2nd day from Monday through Sunday" is actually improved by
"every third day from Monday through Sunday".  I'm also considering
whether to add sanity checks for situations: "9-5" (when it's an hour
field, and when it's not) "8-9/3" (every 3 days from the eighth to the
ninth), and "2-5/0".
...
License: L<perlartistic> ("the Perl Artistic License")

=head1 AUTHOR

Sean M. Burke, E<lt>sburke@cpan.orgE<gt>

=for notes  The "README" section is for PAUSE stuff, not human reading.

=head1 README

Translates crontab notation into English, for sanity checking: For
example, "10 8 15 * * foo bar %baz" into: Run: foo bar with input
...
=item v0.72:  2012-11-09

No changes in behavior-- just tidying up a bit of the code, adding a
few comments, tweaking the docs a bit.  An aside: It has been
I<eleven> years since the first version of this program.  I don't know
if there has been a change in how many people run what types of cron
daemons (Vixie-plus-POSIX; POSIX-only, and
Vixie-plus-POSIX-plus-@stuff).
...
=for html <hr><a href=".">[Back]</a><!-- End of it all -->
...
my $Filter_Prefix = ''; # ...which is a false value
my $POSIX; # whether to be just POSIX

# We have to do switch processing this early, because we
# need the $POSIX flag to be whatever it is, to configure
...
print $Filter_Prefix, " POSIX-only mode\n" if $POSIX;
...
# That's the of top-level code, except for the init blocks below.
# . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .

sub Main {
  # TODO: write unit tests into this, so they run
  #  even before the real code happens?
...
sub Switch_Processing {   # Our dumb switch processing.
  # Only side-effects: $Posix and $Filter_Prefix (and @ARGV)

  # TODO: go "by the book" and change to using Getopt::Std?

  my $Mini_Usage = 
    "Usage: crontab2english [-f] [files]\n"
  ...
  while(@ARGV and $ARGV[0] =~ m<^->s) {
    if($ARGV[0] eq '--') { # end of switches.
      ...
      shift @ARGV;
      $Filter_Prefix = '#> '; # ...which is a true value!
...
sub process_lines {
  ...
  my(@bits,$k,$v);
  foreach (@lines) {
    print $_ if $Filter_Prefix;
    chomp;
    DEBUG > 1 and print "Line: <$_>\n";
    ++$Line_Number;
    next if m/^[ \t]*#/s or m/^[ \t]*$/s; # skipping comment + blank lines.
    ...
    if(!$POSIX and m/^([^= \t]+)[ \t]*=[ \t]*\"(.*)\"[ \t]*$/s ) {
      # NAME = "VALUE"
      $k =~ s/[ \t]+$//;
      $Filter_Prefix or print "Setting env var $k to \"$v\"\n";
    } elsif(!$POSIX and m/^([^= \t]+)[ \t]*=[ \t]*\'(.*)\'[ \t]*$/s ) {
      # NAME = 'VALUE'
      ($k,$v) = ($1,$2);
      #$k =~ s/[ \t]+$//;
    ...
    # And now the workhorse:
    } elsif(!$POSIX and m/^\@(\w+)[ \t]+(.*)/s and exists $atword{lc $1}) {
      process_one_command($_, $atword{lc $1}, $2);
    } elsif( (@bits = split m/[ \t]+/, $_, 6) and @bits == 6 ) {
      DEBUG and print "Bits: ", map("<$_> ", @bits), "\n";
      process_one_command($_, @bits);

    } else {
      if($Filter_Prefix) {
        print $Filter_Prefix, "UNPARSEABLE LINE?!\n";
      } else {
...
sub bits_to_english {
  ...
  # The older and eldritch among you might construe this as sort of a
  # parody of bad old Lisp style of data-structure handling and you
